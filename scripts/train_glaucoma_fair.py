...
parser.add_argument('--dataset', default='fairvision', type=str, help='fairvision|harvardgf|fairdomain')
...
def train(model, criterion, optimizer, scaler, train_dataset_loader, epoch, total_iteration, dataset_info=None, args=None):
    ...
    for i, (input, target, attr) in enumerate(train_dataset_loader):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16): 
            ...
            if isinstance(attr, list):
                attr = torch.cat([a.unsqueeze(0) if a.dim() == 0 else a for a in attr], dim=0)
    ...
    print(f"train ====> epoch {epoch} loss: {np.mean(loss_batch):.4f} auc: {cur_auc:.4f} time: {t2 - t1:.4f}")

    return np.mean(loss_batch), acc, cur_auc, preds, gts, attrs  
    

def validation(model, criterion, optimizer, validation_dataset_loader, epoch, result_dir=None, dataset_info=None, args=None):
    global device
...
if __name__ == '__main__':
    ...
    if args.dataset == "fairvision":

        train_dataset = FairVision_Glaucoma(os.path.join(args.data_dir, 'train'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type)

        test_dataset = FairVision_Glaucoma(os.path.join(args.data_dir, 'test'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type)
        
    if args.dataset == "harvardgf": 
    
          train_dataset = EyeFair(os.path.join(args.data_dir, 'train'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type)
          test_dataset = EyeFair(os.path.join(args.data_dir, 'test'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type)
          
    if args.dataset == "fairdomain": 
        train_dataset = Harvard_DR_Fairness(os.path.join(args.data_dir, 'train'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type)
        test_dataset = Harvard_DR_Fairness(os.path.join(args.data_dir, 'train'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type)
        

    logger.log(f'trn patients {len(train_dataset)} with {len(train_dataset)} samples, test patients {len(test_dataset)} with {len(test_dataset)} samples')
    ...
    assert args.modality_types in ['slo_fundus', 'oct_bscans', 'rnflt', 'oct_fundus']
    if args.model_type == 'ViT-B':
            ...
            if args.vit_weights != 'scratch':
                if args.vit_weights == 'imagenet':
                    ...
                    if args.vit_weights == 'mae':
                        checkpoint_path = "/ddnB/project/mshi/rauniyar/FairVision/scripts/mae_pretrain_vit_base.pth"
                        ...
                        checkpoint_path = "/ddnB/project/mshi/rauniyar/FairVision/scripts/vit-b_CXR_0.5M_mae.pth"
                    elif args.vit_weights == 'mae_color_fundus':
                        checkpoint_path = "/ddnB/project/mshi/rauniyar/FairVision/scripts/RETFound_cfp_weights.pth"
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        test_loss, test_acc, test_auc, test_preds, test_gts, test_attrs = validation(model, criterion, optimizer, test_dataset_loader, epoch, dataset_info=ds_info, args=args)
        
        if args.model_type != 'ViT-B': # we use per-iter lr scheduler for ViT
            scheduler.step()
            
        test_es_acc, test_es_auc, test_aucs_by_attrs, test_dpds, test_eods, test_between_group_disparity = evalute_comprehensive_perf(test_preds, test_gts, test_attrs)
    ...
    best_acc_std, best_es_acc_std, best_auc_std, best_es_auc_std, best_auc_groups_std, best_dpd_groups_std, best_eod_groups_std, best_between_group_disparity_std = bootstrap_performance(best_test_preds_test, best_test_gts_test, best_test_attrs_test, num_classes=2, bootstrap_repeat_times=args.bootstrap_repeat_times)
